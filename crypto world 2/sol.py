...
""" 
    this challenge is same a the first but with authentication
    but the vulnerabilty here is that the authentication mechanism is weak
    You just need to use a username and the server will respond with a token
    Traget user : Kamel (from /api/debug)
"""
...
r=requests.get(base_url,headers={"Authorization": f"Bearer {token}"}) # We know it' a bearer token from the /docs endpoint
print(r.json())
